...
Persistent cache filenames are based on the source content, normalized transformation options, and next-img pipeline version—not the installed Sharp version. Existing Sharp 0.35.3-era filenames remain compatible; that version marker is retained only as a frozen cache-schema value. The cache stores the actual Sharp/libvips/codec fingerprint once in `.next-img-cache.json`. When that toolchain changes, ordinary builds keep using the reviewed cache and ask you to run `next-img`; the cleanup build then regenerates active entries in place and updates the manifest. Identical output bytes therefore create no Git diff. Use `cache.version` as an explicit application-level invalidation value for custom processing changes.
...
The shared loader auto-orients input images and writes optimized content-addressed files into the local cache. Persistent cache keys include the complete transformation, application cache version, and next-img pipeline version. Toolchain versions are tracked separately so dependency upgrades do not rename every committed cache entry. Webpack emits staged asset imports; Turbopack emits stable proxy imports whose asset loader returns the cached optimized bytes. This keeps first-time cold builds reliable even though Turbopack snapshots resolvable paths before loaders finish. The browser downloads only the candidate selected by `<picture>`, `media`, `sizes`, and `srcset`.
