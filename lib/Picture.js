...
const Picture = React.forwardRef(function Picture(
  ...
  const prioritized = priority || preload
  const resolvedLoading = loading ?? (prioritized ? 'eager' : undefined)
  const resolvedDecoding = decoding ?? (prioritized ? 'sync' : 'async')
  const resolvedFetchPriority = fetchPriority ?? (prioritized ? 'high' : undefined)
  ...
  return h(
    React.Fragment,
    null,
    createPreloads(pictureSources, autoSizes, resolvedLoading, {
...
function createPreloads(pictureSources, autoSizes, loading, requestOptions) {
  ...
  const preloads = sources.map(source => {
    ...
    const options = {
      ...
      imageSizes: resolveSizes(source, autoSizes, loading),
      ...(source.media ? { media: source.media } : {}),
