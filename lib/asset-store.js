...
async function cached(create, cacheKey, config, progressMessage) {
  ...
  const manifest = config.persistentCache && config.processing ? await readManifest(cacheDir) : null
  const processingChanged = Boolean(
    config.persistentCache && config.processing && !sameProcessing(manifest?.processing, config.processing),
  )
  const refresh = Boolean(config.rebuildSession && processingChanged)
  ...
  if (cachedImage) {
    ...
    if (processingChanged && !config.rebuildSession) warnProcessingChanged(cacheDir, config)
    await recordUsed(cacheDir, cacheKey, config.rebuildSession, config.processing)
    return cachedImage
  }
