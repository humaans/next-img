...
const warnedToolchains = new Set()
...
async function cached(create, cacheKey, config, progressMessage) {
  ...
  const manifest = config.persistentCache && config.toolchain ? await readManifest(cacheDir) : null
  const toolchainChanged = Boolean(
    config.persistentCache && config.toolchain && !sameToolchain(manifest?.toolchain, config.toolchain),
  )
  const refresh = Boolean(config.rebuildSession && toolchainChanged)
  ...
  if (cachedImage) {
    ...
    if (toolchainChanged && !config.rebuildSession) warnToolchainChanged(cacheDir, config)
    await recordUsed(cacheDir, cacheKey, config.rebuildSession, config.toolchain)
    return cachedImage
  }
...
async function gc(session) {
  ...
  try {
    for (const entry of configs.filter(entry => entry.isDirectory())) {
      ...
      if (config.toolchain) used.add(MANIFEST_FILE)
      await pruneStore(storeDir, used)
      if (config.toolchain) await writeManifest(storeDir, config.toolchain)
    }
  } finally {
...
async function recordUsed(storeDir, fileName, session, toolchain) {
  if (!session) return

  const configRoot = path.join(getGcRoot(session), hash(storeDir))
  const filesRoot = path.join(configRoot, 'files')
  await fs.mkdir(filesRoot, { recursive: true })
  await writeFileAtomic(path.join(configRoot, 'config.json'), JSON.stringify({ storeDir, toolchain }))
...
async function writeManifest(cacheDir, toolchain) {
  const manifest = { version: 1, toolchain }
  await fs.mkdir(cacheDir, { recursive: true })
...
function sameToolchain(previous, current) {
  return previous && JSON.stringify(previous) === JSON.stringify(current)
}

function warnToolchainChanged(cacheDir, config) {
  if (warnedToolchains.has(cacheDir)) return
  warnedToolchains.add(cacheDir)
  const warn = config.warn || console.warn
  warn('next-img cache uses a different Sharp toolchain. Run `next-img` to refresh it in place.')
}
