  ...
  for (const size of sizes) {
    for (let density of config.densities) {
      ...
      const resize = async (outputFormat, outputOptions) => {
        ...
        const assetPath = await materializeAsset(fileName, data, config)
...
async function materializeAsset(fileName, data, config) {
...
async function gc(session = process.env.NEXT_IMG_REBUILD) {
  if (!session) {
