...
async function load(filePath, buffer) {
  ...
  const config = merge({}, loaderConfig, queryConfig, {
    processing: {
      ...
      ...(loaderConfig.cacheVersion != null && { cacheVersion: loaderConfig.cacheVersion }),
  ...
  validateBareImportSize(loaderContext, filePath, originalFileName, metadata, loaderConfig, queryConfig)
  ...
  for (const { size, density, width } of plan.candidates) {
    const resize = async (outputFormat, outputOptions) => {
      ...
      const cacheContent = createCacheContent(buffer, config, outputOptions, {
        ...
        ...(config.cacheVersion != null && { cacheVersion: config.cacheVersion }),
        autoOrient: true,
        width,
        format: outputFormat,
...
async function createPlaceholder({ buffer, config, originalFileName, createFileName }) {
  ...
  const cacheContent = createCacheContent(
    buffer,
    config,
    {
      width,
...
function getAssetProxyPath(proxyDir, format) {
  ...
  return path.join(proxyDir, `generated.${format === 'jpeg' ? 'jpg' : format}`)
}
...
function createCacheContent(buffer, config, persistentIdentity, ephemeralIdentity) {
  ...
  const serializedIdentity = config.persistentCache
    ? JSON.stringify(persistentIdentity)
    : canonicalStringify(ephemeralIdentity)
  return Buffer.concat([buffer, Buffer.from(serializedIdentity, 'utf8')])
}

function validateBareImportSize(loaderContext, filePath, originalFileName, metadata, loaderConfig, queryConfig) {
  ...
  if (loaderConfig.strict) throw new Error(message)

  const warningKey = `${filePath}:${width}:${height}:${limit}`
  if (warnedBareImports.has(warningKey)) return
  warnedBareImports.add(warningKey)
  if (typeof loaderContext.emitWarning === 'function') loaderContext.emitWarning(new Error(message))
