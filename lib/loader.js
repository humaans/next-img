...
async function load(filePath, buffer) {
  ...
  for (const { size, density, width } of plan.candidates) {
    const resize = async (outputFormat, outputOptions) => {
      ...
      const cacheContent = createCacheContent(buffer, outputOptions)
...
function createCacheContent(buffer, codecOptions) {
  return Buffer.concat([buffer, Buffer.from(JSON.stringify(codecOptions), 'utf8')])
}
...
function validateBareImportSize(filePath, originalFileName, metadata, loaderConfig, queryConfig, report) {
  ...
  if (limit === false || queryConfig.sizes) return
  ...
  const message =
    `next-img imported ${originalFileName} at its intrinsic ${width}×${height} size because no sizes were provided. ` +
    `Add ?sizes=..., increase nextImg.maxBareImportSize above ${limit}, or set it ` +
...
function createReporter(loaderContext, strict) {
  return message => {
    ...
    // oxlint-disable-next-line anti-slop/no-runtime-typeof -- Turbopack exposes only part of Webpack's loader context.
