...
// Preserve the cache-key schema introduced before toolchain versions moved to
// the manifest. This value is intentionally frozen and is not the installed
// Sharp version; changing it would rename every persistent cache entry.
const PERSISTENT_CACHE_KEY_SHARP_VERSION = '0.35.3'
    ...
    const resize = async (outputFormat, outputOptions) => {
      ...
      const transform = {
        ...
        ...(config.persistentCache
          ? { sharpVersion: PERSISTENT_CACHE_KEY_SHARP_VERSION }
