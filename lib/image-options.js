...
const OUTPUT_FORMATS = ['avif', 'webp', 'jpeg', 'png']
...
function parseResourceQuery(query, { strict = false, warn = console.warn } = {}) {
  if (!query) return {}
...
function createTransformPlan(config, metadata) {
  ...
  if (![false, true, 'blur'].includes(config.placeholder || false)) {
    throw new Error('next-img option "placeholder" must be "blur", true, or false')
  }
  if (config.placeholder) validateNumberList([config.placeholderSize || 16], 'placeholderSize', { integer: true })
