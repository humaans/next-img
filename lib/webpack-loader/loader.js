  ...
  for (let size of config.sizes) {
    ...
    for (let density of config.densities) {
      ...
      const resize = async output => {
        ...
        console.log('read from cache', Date.now() - t1)
      ...
      const [orig, webp] = await Promise.all([resize(metadata.format), resize('webp')])
...
async function cached(fn, cacheKey, config) {
  ...
  try {
    ...
    console.log('Read file', Date.now() - t)
    const t2 = Date.now()
    const info = await sharp(buffer).metadata()
    console.log('Sharped', Date.now() - t2)
...
async function gc() {
  console.log('clense', gcConfig.processed)
